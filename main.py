              ...
              if i % cfg.wandb.log.frequency == 0:
                    ...
                    if cfg.wandb.log.img and i % 100 == 0 and cfg.model.type == "score":
                      ...
                      def get_sample(t, key1, key0, xt, text_embedding):
                        return sample(jnp.array(1e-5,dtype=jnp.float32), jnp.array(0,dtype=jnp.float32), t.astype(float)[0], jnp.array([-1/1000]).astype(float)[0], drift_test, diffusion_test, [inference_model, text_embedding,model_parameters if cfg.model.name != "sde" else data[0], key0], xt, key1, named_sharding) 
