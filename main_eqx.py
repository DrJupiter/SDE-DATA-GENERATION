...
# Model and optimizer
from models.ddpm.ddpm_eqx import ddpm 
...
def run_experiment(cfg):
    ...
    model = ddpm(cfg, key = subkey) # model_call(x_in, timesteps, parameters)
    model_parameters = None

    # Get optimizer and its parameters
    # optimizer, optim_parameters = get_optim(cfg, model_parameters)
    optimizer = optax.adam(1e-4)
    optim_parameters = optimizer.init(model)
    ...
    for epoch in range(cfg.train_and_test.train.epochs): 
        for i, (data, labels) in enumerate(train_dataset): # batch training
            ...
            print(model(perturbed_data[0], scaled_timesteps[0], subkey[2]))
            ...
            grads = grad_fn(model, model_parameters, perturbed_data, scaled_timesteps, subkey[2])
            ...
            if i % cfg.wandb.log.frequency == 0:
                  if cfg.wandb.log.loss:
                    wandb.log({"loss": loss_fn(model, model_parameters, perturbed_data, scaled_timesteps, subkey[2])})
                    # wandb.log({"loss": loss_value})
                  if cfg.wandb.log.img:
                     display_images(cfg, model(perturbed_data, scaled_timesteps, model_parameters, subkey[2]), labels)
        # Test loop
