...
def get_lin_log_softmax(cfg, key):
    abf = cfg.model.hyperparameters.anti_blowup_factor
    embedding_dim = len(cfg.dataset.classes) 
    shape = jnp.array(cfg.dataset.shape)+jnp.array([0,cfg.dataset.padding*2,cfg.dataset.padding*2,0])
    image_size = abs(jnp.prod(shape))
    ...
    subkey = random.split(key,4)
    params["w0"] = initializer(subkey[0], (image_size, embedding_dim), dtype=jnp.float32)
    params["b0"] = initializer(subkey[1], (1, embedding_dim), dtype=jnp.float32)

    def lin_log_softmax(data, params):
        out = jnp.matmul(data, params["w0"]) + params["b0"]
        out = nn.log_softmax(out)
        return out
    
    return lin_log_softmax, params
